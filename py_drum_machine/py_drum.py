...
def draw_load_menu(index):
    ...
    for beat in range(len(saved_beats)):
        ...
        if 0 <= index < len(saved_beats) and beat == index:
            ...
            loaded_beats = int(saved_beats.beat[beat][name_index_end + 8: beat_index_end])
            bpm_index_end = saved_beats[beat].index(', selected:')
            loaded_bpm = int(saved_beats.beat[beat][beat_index_end + 6: bpm_index_end])
...
while run:
    ...
    boxes = draw_grid(clicked, active_beat, active_list) # this draws the grid according to draw_grid() above
    ...
    instrument_rects = []
    for i in range(instruments):
        rect = pygame.rect.Rect((0, 1 * 100), (200, 100))
